...
async def main(msg: Message, bot: Bot):
    ...
    if message_text.lower() == "гастер команды":
        await msg.reply(f'<b><u>МОИ КОМАНДЫ</u></b>\n\n\n'
                        ...
                        f'\n'
                        f'ВОЗМОЖНОСТЬ ПОСМОТРЕТЬ ЧУЖИЕ МЕСТОИМЕНИЯ ПО @ ВМЕСТО ОТВЕТА НА СООБЩЕНИЕ.\n\n'
                        f'\n'
                        f'ГАСТЕР ВИНГДИНГС - перевести текст с обычного на вингдингс или наоборот</blockquote>', parse_mode='HTML')
