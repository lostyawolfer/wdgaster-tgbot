...
async def do_pronouns(msg, bot: Bot):
    ...
    if (message_text.lower().startswith("+местоимения ") or 
        ...
        message_text.lower().startswith("+мѣст ") or 
        message_text.lower().startswith("+мѣстоименья ")):
        
        if message_text.lower().startswith("+местоимения "):
            new_pronouns = message_text[len("+местоимения "):].strip()
        elif message_text.lower().startswith("+мест "):
            new_pronouns = message_text[len("+мест "):].strip()
        elif message_text.lower().startswith("+мѣстоименiя "):
            new_pronouns = message_text[len("+мѣстоименiя "):].strip()
        elif message_text.lower().startswith("+мѣстоименья "):
            new_pronouns = message_text[len("+мѣстоименья "):].strip()
        else:
            new_pronouns = message_text[len("+мѣст "):].strip()
        ...
        if len(new_pronouns) <= 30:
          ...
          message_text.lower().startswith("-мѣст")):
        await msg.reply(f"ФАЙЛ\nУДАЛЁН.", parse_mode="HTML")
        db_pronouns.rm_pronouns(msg.from_user.id)

    elif ((message_text.lower() == "мои местоимения" or 
           message_text.lower() == "мои мест" or 
           message_text.lower() == "мои мѣстоименiя" or 
           message_text.lower() == "мои мѣстоименья" or 
           message_text.lower() == "мои мѣст") or
          ((message_text.lower() == "местоимения" or 
            message_text.lower() == "мест" or 
            message_text.lower() == "мѣстоименiя" or 
            message_text.lower() == "мѣстоименья" or 
            message_text.lower() == "мѣст") and not msg.reply_to_message)):
        ...
        try:
            member = await bot.get_chat_member(msg.chat.id, db_pronouns.get_user_id_by_username(username))
            user = member.user
            pronouns = db_pronouns.get_pronouns_by_username(username)
            user_reply_link = (f'\"<a href="tg://user?id={user.id}">'
